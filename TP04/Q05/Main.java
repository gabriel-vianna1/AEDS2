...
class Hash{
    int TAM1, TAMreserva, totalReserva, TAM;
    Pokemon[] tabela;
    ...
    if(p == null){
        throw new Exception("Objeto recebido é nulo");
    }
     ...
     //Verifica se a reserva não está cheia
     else if(totalReserva < TAMreserva){
        //Insere na primeira posição disponível na reserva
        tabela[TAM1 + totalReserva] = Pokemon.clonePokemon(p);
        totalReserva++;
     }
   }
   ...
   public int pesquisar(String palavra){
    ...
    if(tabela[pos] != null){
     if(tabela[pos].getName().equals(palavra)){
        ...
        //Vai perecorrer a reserva até encontrar
        for(int i = 21; i < TAM; i++){
            if(tabela[i].getName().equals(palavra)){
                resp = i;
                i = TAM;//para se for encontrado
       }
      } 
     }
    }
