...
class Main {
    public static void main(String[] args) {
        ...
        int contador = Pokemon.insertionSort(pokemonArray);
        ...
        try (PrintWriter escritor = new PrintWriter(new FileWriter("matrícula_inserção.txt"))) {
...
class Pokemon {
    ...
    .collect(Collectors.joining(", "));  // Juntando as habilidades separadas por vírgula
     ...
     System.out.println(p.getIsLegendary() + " - " + p.getGeneration() + " gen" + "]" + " - " + formattedDate);
        ...
        public static int insertionSort(Pokemon[] array) {
            int cont = 0;
            for (int i = 1; i < array.length; i++) {
                Pokemon key = Pokemon.clonePokemon(array[i]);
                int j = i - 1;
            // Se os pokemons tiverem a mesma data de captura, ele compara por nome.
             if(array[j].getCaptureDate().isEqual(key.getCaptureDate())){
                 if(array[j].getName().compareTo(key.getName()) > 0){
                    Pokemon tmp = Pokemon.clonePokemon(array[j]);
                    array[j] = Pokemon.clonePokemon(key);
                    key = Pokemon.clonePokemon(tmp);
                 }      
                 j--;
             }  
             
             while (j >= 0 && array[j].getCaptureDate().isAfter(key.getCaptureDate())) {
                    array[j + 1] = Pokemon.clonePokemon(array[j]);
                    j--;
                    cont++;               
                }
                
                 array[j + 1] = Pokemon.clonePokemon(key);
                cont++;
            }
            return cont;
        }
